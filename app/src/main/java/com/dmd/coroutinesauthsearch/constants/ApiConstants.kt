package com.dmd.coroutinesauthsearch.constants

object ApiConstants {
    const val BASE_URL = "https://webdemo.hensoldt-analytics.com/api/"
    const val AUTH = "auth/auth"
    const val BASIC_AUTH = "auth/basicAuth"
    const val AUTH_TOKEN = "auth/authToken"
    const val BASIC_AUTH_TOKEN = "auth/basicAuthToken"
    const val AUTH_INFO = "auth/authInfo" //get
    const val PROMOTE = "auth/promote"
}